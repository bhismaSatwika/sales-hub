...
class c_subsidiary_inventory_sales_order_dropship(object):
    ...
    async def create(
        ...
        await self.validasi_approval(data["salesman"])
    ...
    async def validasi_approval(self, salesman):
        ...
        message = "Belum ada approver untuk salesman ini"
        raise HTTPException(400, message)

    async def receive(self, data):
        update_delivery = self.db.genUpdateObject(
            {"is_delivered": True},
            {"id_trans": data["id_delivery"]},
            "trans_inventory_holding_delivery_preparation_header",
        )

        try:
            trans = await self.db.executeTrans([update_delivery])
            if trans["status"] == False:
                raise HTTPException(
                    400, ("error ketika request approval: ", trans["message"])
                )
...
@app.post("/api/f_trans/c_subsidiary_inventory_sales_order_dropship/receive")
