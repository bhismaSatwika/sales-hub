...
class c_master_kecamatan(object):
    def __init__(self):
        self.db = Db()
    ...
    async def read(self, orderby, limit, offset, filter, filter_other="", filter_other_conj=""):
        ...
        sql = "SELECT *,ROW_NUMBER() OVER (ORDER BY id DESC) AS nomor_urut from master_kecamatan"+str_clause
        sql_count = (
    ...
    async def create(self, data):
        ...
        sqlString = self.db.genStrInsertSingleObject(data,"master_kecamatan")
    ...
    async def update(self, data, data_where):
        ...
        sqlString = self.db.genUpdateObject(data,data_where,"master_kecamatan")
        # print(sqlString)
        try:
            await self.db.executeQuery(sqlString)
            message = {"status": "success"}
    ...
    async def delete(self, data_where):
        sqlString = self.db.genDeleteObject(data_where,"master_kecamatan")
        try:
            await self.db.executeQuery(sqlString)
            message = {"status": "success"}
        except Exception as e:
    ...
    async def get_kecamatan_kode(self,kode_prov_kode_kotakab):
        sql = f"""SELECT kode_kec as value, nama as text,trim(kode_prov||kode_kotakab||kode_kec) as kode_concat 
                FROM master_kecamatan
                WHERE trim(kode_prov||kode_kotakab) = trim('{kode_prov_kode_kotakab}') 
...
@app.get("/api/f_master/c_master_kecamatan/read")
async def read_data(
    ...
    ob_data = c_master_kecamatan()
    return await ob_data.read(orderby, limit, offset, filter)


@app.post("/api/f_master/c_master_kecamatan/create")
...
@app.post("/api/f_master/c_master_kecamatan/update")
...
@app.post("/api/f_master/c_master_kecamatan/delete")
...
@app.get("/api/f_master/c_master_kecamatan/get_kecamatan_kode")
async def get_kecamatan_kode(param: object = Query(None, alias="param")):
    data = json.loads(param)
    ob_data = c_master_kecamatan()
    return await ob_data.get_kecamatan_kode(data)
