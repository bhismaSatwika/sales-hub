...
class c_master_provinsi(object):
    def __init__(self):
        self.db = Db()
    ...
    async def read(self, orderby, limit, offset, filter, filter_other="", filter_other_conj=""):
        ...
        sql = "SELECT *,ROW_NUMBER() OVER (ORDER BY id DESC) AS nomor_urut from master_provinsi"+str_clause
        sql_count = (
            "SELECT count(*) as count FROM master_provinsi"
            + str_clause_count
        )
    ...
    async def create(self, data):
        ...
        sqlString = self.db.genStrInsertSingleObject(data,"master_provinsi")
    ...
    async def update(self, data, data_where):
        ...
        sqlString = self.db.genUpdateObject(data,data_where,"master_provinsi")
        # print(sqlString)
        try:
            await self.db.executeQuery(sqlString)
            message = {"status": "success"}
    ...
    async def delete(self, data_where):
        sqlString = self.db.genDeleteObject(data_where,"master_provinsi")
        try:
            await self.db.executeQuery(sqlString)
            message = {"status": "success"}
        except Exception as e:
    ...
    async def get_provinsi_kode(self):
        sql = f""" select kode_prov as value, nama as text,id as provinsi_id from master_provinsi 
...
example /testing url
test from postman :
url/api/c_master_provinsi/testing
for post method and other method, check tutorial from 
https://fastapi.tiangolo.com/
"""

@app.get("/api/f_master/c_master_provinsi/read")
async def read_data(
    ...
    ob_data = c_master_provinsi()
    return await ob_data.read(orderby, limit, offset, filter)


@app.post("/api/f_master/c_master_provinsi/create")
...
@app.post("/api/f_master/c_master_provinsi/update")
...
@app.post("/api/f_master/c_master_provinsi/delete")
...
@app.get("/api/f_master/c_master_provinsi/get_provinsi_kode")
async def get_provinsi_kode():
    ob_data = c_master_provinsi()
    return await ob_data.get_provinsi_kode()
