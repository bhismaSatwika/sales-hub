...
class c_master_cabang(object):
    ...
    async def read(
        ...
        sql = (
            "SELECT aa.*,ROW_NUMBER() OVER (ORDER BY aa.id_cabang DESC) AS nomor_urut,bb.company_name,bb.kode FROM master_company_cabang aa LEFT JOIN master_company bb ON aa.id_company = bb.id_company"
            + str_clause
        )
        sql_count = (
    ...
    async def get_cabang_where_condition(self, where_condition):
        ...
        sql = f"""SELECT id_cabang as value,cabang_name as text, is_pusat 
