...
class c_master_produk(object):
    ...
    async def get_ppn_pph22(self, id_produk):
        sql = f"""SELECT a.ppn, b.presentase as pph22 FROM master_produk A
        JOIN master_pajak B on b.pajak = 'pph22' and b.status_aktif = true
        WHERE id_produk = {id_produk} and a.status_aktif = true"""
        result = await self.db.executeToDict(sql)
        return result[0]
