...
class c_master_approval_header(object):
    ...
    async def read(
        ...
        query = """SELECT A.*, B.nama_approval FROM master_approval_header A
        LEFT JOIN master_approval_type B on A.approval_type_id = B.id_type"""

        sql = query + str_clause
        sql_count = query + str_clause_count

        sql_count = f"""SELECT COUNT(*) FROM ({sql_count}) as count"""
