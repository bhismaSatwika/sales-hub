...
class c_master_company(object):
    ...
    async def read(self, orderby, limit, offset, filter, filter_other="", filter_other_conj=""):
        ...
        sql = "SELECT *,ROW_NUMBER() OVER (ORDER BY id_company DESC) AS nomor_urut from master_company"+str_clause
        sql_count = (
            "SELECT count(*) as count FROM master_company"
            + str_clause_count
        )
    ...
    async def create(self, data):
        ...
        sqlString = self.db.genStrInsertSingleObject(data,"master_company")
    ...
    async def update(self, data, data_where):
        ...
        sqlString = self.db.genUpdateObject(data,data_where,"master_company")
        # print(sqlString)
        try:
            await self.db.executeQuery(sqlString)
            message = {"status": "success"}
    ...
    async def delete(self, data_where):
        sqlString = self.db.genDeleteObject(data_where,"master_company")
        try:
            await self.db.executeQuery(sqlString)
            message = {"status": "success"}
        except Exception as e:
    ...
    async def get_company(self):
        sql = f"""SELECT id_company as value,company_name as text 
				    FROM master_company 
    ...
    async def get_company_where_condition(self,where_condition):
    ...
    async def get_atribut_company(self, id_company):
        sql = f"""SELECT id_company as value,company_name as text,* FROM master_company 
        WHERE id_company = {id_company} AND status_release = 't' AND status_aktif = 't'
        LIMIT 1"""
        result = await self.db.executeToDict(sql)
        data = {
            "data": result
...
example /testing url
test from postman :
url/api/c_master_company/testing
for post method and other method, check tutorial from 
https://fastapi.tiangolo.com/
"""

@app.get("/api/f_master/c_master_company/read")
async def read_data(
    ...
    ob_data = c_master_company()
    return await ob_data.read(orderby, limit, offset, filter)


@app.post("/api/f_master/c_master_company/create")
...
@app.post("/api/f_master/c_master_company/update")
...
@app.post("/api/f_master/c_master_company/delete")
async def delete(request:Request):
    data = await request.json()
    ob_data = c_master_company()
    return await ob_data.delete(data)

@app.get("/api/f_master/c_master_company/get_company")
async def get_company():
    ob_data = c_master_company()
    return await ob_data.get_company()


@app.get("/api/f_master/c_master_company/get_company_where_condition")
...
@app.get("/api/f_master/c_master_company/get_atribut_company")
async def get_atribut_company(param: object = Query(None, alias="param")):
    # print('MASUKKKKKK')
    data = json.loads(param)
    ob_data = c_master_company()
    return await ob_data.get_atribut_company(data)
